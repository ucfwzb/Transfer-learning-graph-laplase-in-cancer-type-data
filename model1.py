...
### Delete the features which have more than 30 0s, since this kind feature is not significant.
    ...
    index_1_var = [x[0]
                   for x in line_1_var][:int(round(len(line_1_var) * 0.5, 0))]
    index_2_var = [x[0]
                   for x in line_2_var][:int(round(len(line_2_var) * 0.5, 0))]
    index_1_mean = [x[0] for x in line_1_mean
                    ][:int(round(len(line_1_mean) * 0.5, 0))]
    index_2_mean = [x[0] for x in line_2_mean
                    ][:int(round(len(line_2_mean) * 0.5, 0))]
...
def cross_valudation(data_1_all, data_2_all, label_1_all, label_2_all,
                     valudation_number, test_number):
    ### get the test part
    ...
    data1_train, data1_valudation, label1_train, label1_valudation = train_test_split(
        data_1, label_1, test_size=valudation_number)
    data2_train, data2_valudation, label2_train, label2_valudation = train_test_split(
        data_2, label_2, test_size=valudation_number)
...
def train_model1(n,data1_train, data1_valudation, label1_train,label1_valudation, \
    ...
    roc1_train, roc2_train = model1_roc(n, f1, f2, data1, data2, label1_train,
                                        label1_valudation, label2_train,
                                        label2_valudation)
...
def base_model1(n, data_train, data_valudation, data_test, label_train,
                label_valudation, label_test):
    alpha = 0.1
    ...
    label_temp = np.concatenate(
        (label_train, np.repeat(0., len(label_test)).reshape(
            (len(label_test)), 1)),
        axis=0)
    y = intial_y(n, data_train, label_temp)
    f = np.dot(
        (1 - alpha) * np.linalg.inv(np.eye(n) - laplace_matrix(n, data_train)),
        y)
...
def base_model1_roc(n,data1_train, data1_valudation, data1_test, label1_train,label1_valudation, label1_test,\
           data2_train, data2_valudation, data2_test, label2_train, label2_valudation, label2_test):
    f1, data1_train, label1_train = base_model1(
        n, data1_train, data1_valudation, data1_test, label1_train,
        label1_valudation, label1_test)
    f2, data2_train, label2_train = base_model1(
        n, data2_train, data2_valudation, data2_test, label2_train,
        label2_valudation, label2_test)
    base_roc1, base_roc2 = model1_roc(n, f1, f2, data1_train, data2_train,
                                      label1_train, label1_test, label2_train,
                                      label2_test)
...
def train_model(n,data1_train, data1_valudation, label1_train,label1_valudation, data2_train,\
    ...
    data1 = np.concatenate((data1_train, data1_valudation), axis=0)
    data2 = np.concatenate((data2_train, data2_valudation), axis=0)
    fv1, fv2, fvc, fu1, fu2 = intial_fv_fu(data1, data2, label1_train,
                                           label2_train)
    label1_temp = np.concatenate(
        (label1_train, np.repeat(0., len(label1_valudation)).reshape(
            (len(label1_valudation)), 1)),
        axis=0)
    label2_temp = np.concatenate(
        (label2_train, np.repeat(0., len(label2_valudation)).reshape(
            (len(label2_valudation)), 1)),
        axis=0)
    ...
    fpr, tpr, _ = roc_curve(
        label1_valudation.ravel(), fu1_test.ravel(), pos_label=-1)
    roc_auc1 = auc(fpr, tpr)
    fu2_test = fu2[len(fu2) - len(label2_valudation):]
    fpr, tpr, _ = roc_curve(
        label2_valudation.ravel(), fu2_test.ravel(), pos_label=-1)
...
def base_model2(n, data_train, data_valudation, data_test, label_train,
                label_valudation, label_test):
    alpha = 0.01
...
def base_model2_roc(n,data1_train, data1_valudation, data1_test,\
               ...
               data2_train, data2_valudation,data2_test, label2_train, label2_valudation, label2_test):
    roc_auc1 = base_model2(n, data1_train, data1_valudation, data1_test,
                           label1_train, label1_valudation, label1_test)
    roc_auc2 = base_model2(n, data2_train, data2_valudation, data2_test,
...
def all_models(n,data1_train, data1_valudation, data1_test,\
    ...
    return  model1_1_roc_train_max, final_model1_1_roc, model1_2_roc_train_max, final_model1_2_roc, base_roc1,\
...
def whole_process(n, data_1_all, data_2_all, label_1_all, label_2_all,
                  valudation_number, test_number, preprocess_scale):
    ...
    for i in range(50):
        ...
        final_model1_2_roc,base_roc1, base_roc2, model2_1_roc_train_max,\
        final_model2_1_roc,model2_2_roc_train_max, final_model2_2_roc,base2_roc_auc1,\
        base2_roc_auc2=all_models(n,data1_train, data1_valudation, data1_test,\
                                                                                label1_train,label1_valudation, label1_test,\
                                                                                data2_train, data2_valudation,data2_test,\
                                                                                label2_train, label2_valudation, label2_test)
        ...
        print(i)
        temp_report = open('temp_report_swap_%s_new2.txt' % preprocess_scale,
                           'a')
        ...
        temp_report.close()
    final_report = open('final_report_swap_%s_new2.txt' % preprocess_scale,
                        'w')
...
if __name__ == "__main__":
    ...
    n, data_1_all, data_2_all, label_1_all, label_2_all = read_data(
        preprocess_scale)
